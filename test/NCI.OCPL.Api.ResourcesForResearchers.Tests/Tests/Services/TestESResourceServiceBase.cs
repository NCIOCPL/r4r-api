...
namespace NCI.OCPL.Api.ResourcesForResearchers.Tests.Services
{
    ...
    public class TestESResourceServiceBase
    {
        ...
        /// <param name="callback">Callback method to access request details.</param>
        /// <typeparam name="T">The 1st type parameter.</typeparam>
        protected T GetService<T>(Action<ApiCallDetails> callback) where T : class
        {
            ...
            InMemoryConnection connection = new InMemoryConnection(new byte[0], statusCode: 200);
            var connectionSettings = new ElasticsearchClientSettings(pool, connection)
                .DisableDirectStreaming() // This allows us to read the request and response in the callback
        ...
        /// <typeparam name="T">The 1st type parameter.</typeparam>
        protected T GetService<T>(InMemoryConnection connection) where T : class
        {
